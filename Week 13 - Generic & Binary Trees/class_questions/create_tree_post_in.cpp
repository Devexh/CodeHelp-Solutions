...
TreeNode *constructTreeFromPostAndInorderTraversal(
    map<int, int> &valToIndexMap, int postOrder[], int inOrder[],
    int &postOrderIndex, int inOrderStart, int inOrderEnd, int size) {
  ...
  postOrderIndex--;
  TreeNode *root = new TreeNode(element);
  // seach karo inorder mein
  int position = valToIndexMap[element];
  // baaki recursion sambhal lega
  root->right = constructTreeFromPostAndInorderTraversal(
      valToIndexMap, postOrder, inOrder, postOrderIndex, position + 1,
      inOrderEnd, size);
  root->left = constructTreeFromPostAndInorderTraversal(
      valToIndexMap, postOrder, inOrder, postOrderIndex, inOrderStart,
      position - 1, size);
  return root;
}
...
int main() {
  ...
  //       valToIndexMap, preOrder, inOrder, preOrderIndex, inOrderStart,
  //       inOrderEnd, size);
  ...
  TreeNode *root = constructTreeFromPostAndInorderTraversal(
      valToIndexMap, postOrder, inOrder, postOrderIndex, inOrderStart,
      inOrderEnd, size);
